...
4. Codeman starts the next run right away, and it implements the plan on the task branch. When the agent reports the work as done, Codeman opens a pull request that closes the issue, and the issue gets `codeman:done`. Unfinished work is committed, and the following run continues it.
